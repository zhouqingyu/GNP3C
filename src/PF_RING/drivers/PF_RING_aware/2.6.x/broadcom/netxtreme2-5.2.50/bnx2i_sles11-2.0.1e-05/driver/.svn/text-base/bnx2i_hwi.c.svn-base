...
static void bnx2i_adjust_qp_size(struct bnx2i_hba *hba)
{
	...
	if (test_bit(BNX2I_NX2_DEV_5706, &hba->cnic_dev_type) ||
		...
		if (!is_power_of_2(hba->max_sqes))
			hba->max_sqes = __rounddown_pow_of_two(hba->max_sqes);

		if (!is_power_of_2(hba->max_rqes))
			hba->max_rqes = __rounddown_pow_of_two(hba->max_rqes);
	}
 ...
 *	doorbell register
 */
static void bnx2i_ring_sq_dbell(struct bnx2i_conn *bnx2i_conn, u16 count)
{
					      ...
					      int count)
{
	...
	/* Ring the doorbell */
	bnx2i_ring_sq_dbell(bnx2i_conn, (u16)bnx2i_conn->ep->qp.sq_prod_idx);
}
			 ...
			 struct iscsi_task *mtask)
{
	...
	if (!ctask || !ctask->sc)
		 ...
		 * was starting up.
                 * Note.  In the case of a SCSI cmd timeout, the task's sc is
                 *        still active; hence ctask->sc != 0
		 */
		return 0;
				       ...
				       struct cqe *cqe)
{
	...
	if (resp_cqe->status) {
		...
		if (!data_len) {
			/* CHK_CONDITION len is qualify in libiscsi */
			goto done;
		}
		...
		} else if (data_len > ISCSI_DEF_MAX_RECV_SEG_LEN) {
			iscsi_conn_printk(KERN_ERR, conn,
					  "sense data len %d > conn data\n",
					  data_len);
			data_len = ISCSI_DEF_MAX_RECV_SEG_LEN;
		}
		/* Must consume RQ entry for all data_len > 0 */
		bnx2i_get_rq_buf(bnx2i_cmd->conn, conn->data, data_len);
					...
					struct iscsi_kcqe *new_cqe_kcqe)
{
	...
	if (!conn) {
		printk(KERN_ALERT "fpn - hba %p, cid #%x not valid\n", hba, iscsi_cid);
		return;
	}
	if (!conn->ep) {
		printk(KERN_ALERT "fpn - cid #%x - ep not bound\n", iscsi_cid);
		return;
	}
				    ...
				    struct iscsi_kcqe *tcp_err)
{
	...
	if (!bnx2i_conn) {
		printk(KERN_ALERT "bnx2i - tcp err - cid 0x%x not valid\n", iscsi_cid);
		return;
	}
				      ...
				      struct iscsi_kcqe *iscsi_err)
{
	...
	bnx2i_conn = bnx2i_get_conn_from_id(hba, iscsi_cid);
	if (!bnx2i_conn) {
		printk(KERN_ALERT "bnx2i - conn err, cid 0x%x not valid\n", iscsi_cid);
		return;
	}

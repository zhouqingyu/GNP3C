 ...
 */
void fc_get_host_port_state2(struct Scsi_Host *shost)
{
...
EXPORT_SYMBOL(fc_get_host_port_state2);

/**
 * fc_get_host_speed() - Return the speed of the given Scsi_Host
 * @shost: The SCSI host whose port speed is to be determined
 */
void fc_get_host_speed2(struct Scsi_Host *shost)
{
	struct fc_lport *lport = shost_priv(shost);

	fc_host_speed(shost) = lport->link_speed;
}
EXPORT_SYMBOL(fc_get_host_speed2);

/**
 * fc_get_host_stats2() - Return the Scsi_Host's statistics
 * @shost: The SCSI host whose statistics are to be returned
 */
struct fc_host_statistics *fc_get_host_stats2(struct Scsi_Host *shost)
{
...
EXPORT_SYMBOL(fc_get_host_stats2);
				   ...
				   struct fc_frame *in_fp)
{
	...
	if (fp) {
		...
		*((__be32 *)dp) = htonl(ELS_LS_ACC << 24);
		fc_fill_reply_hdr2(fp, in_fp, FC_RCTL_ELS_REP, 0);
...
/**
 * fc_fabric_login2() - Start the lport state machine
 * @lport: The local port that should log into the fabric
 *
 * Locking Note: This function should not be called
 *		 with the lport lock held.
 */
int fc_fabric_login2(struct fc_lport *lport)
{
	int rc = -1;
...
EXPORT_SYMBOL(fc_fabric_login2);

/**
 * __fc_linkup2() - Handler for transport linkup events
 * @lport: The lport whose link is up
 *
 * Locking: must be called with the lp_mutex held
 */
void __fc_linkup2(struct fc_lport *lport)
{
	if (!lport->link_up) {
		lport->link_up = 1;
...
/**
 * fc_linkup2() - Handler for transport linkup events
 * @lport: The local port whose link is up
 */
void fc_linkup2(struct fc_lport *lport)
{
...
EXPORT_SYMBOL(fc_linkup2);

/**
 * __fc_linkdown2() - Handler for transport linkdown events
 * @lport: The lport whose link is down
 *
 * Locking: must be called with the lp_mutex held
 */
void __fc_linkdown2(struct fc_lport *lport)
{
	if (lport->link_up) {
		lport->link_up = 0;
		fc_lport_enter_reset(lport);
...
/**
 * fc_linkdown2() - Handler for transport linkdown events
 * @lport: The local port whose link is down
 */
void fc_linkdown2(struct fc_lport *lport)
{
	printk(KERN_INFO "host%d: libfc: Link down on port (%6.6x)\n",
	       lport->host->host_no, lport->port_id);

	mutex_lock(&lport->lp_mutex);
	__fc_linkdown2(lport);
	mutex_unlock(&lport->lp_mutex);
}
EXPORT_SYMBOL(fc_linkdown2);

/**
 * fc_fabric_logoff2() - Logout of the fabric
 * @lport: The local port to logoff the fabric
 *
 * Return value:
 *	0 for success, -1 for failure
 */
int fc_fabric_logoff2(struct fc_lport *lport)
{
...
EXPORT_SYMBOL(fc_fabric_logoff2);

/**
 * fc_lport_destroy2() - Unregister a fc_lport
 ...
 *
 */
int fc_lport_destroy2(struct fc_lport *lport)
{
	mutex_lock(&lport->lp_mutex);
...
EXPORT_SYMBOL(fc_lport_destroy2);

/**
 * fc_set_mfs2() - Set the maximum frame size for a local port
 * @lport: The local port to set the MFS for
 * @mfs:   The new MFS
 */
int fc_set_mfs2(struct fc_lport *lport, u32 mfs)
{
 ...
 */
void fc_lport_set_local_id2(struct fc_lport *lport, u32 port_id)
{
	mutex_lock(&lport->lp_mutex);
...
EXPORT_SYMBOL(fc_lport_set_local_id2);
				    ...
				    struct fc_frame *rx_fp)
{
	...
	if (fp) {
		 ...
		 * repeat the sequence.
		 */
		fc_fill_reply_hdr2(fp, rx_fp, FC_RCTL_ELS_REP, 0);
...
/**
 * fc_lport_reset2() - Reset a local port
 * @lport: The local port which should be reset
 *
 * Locking Note: This functions should not be called with the
 *		 lport lock held.
 */
int fc_lport_reset2(struct fc_lport *lport)
{
	cancel_delayed_work_sync(&lport->retry_work);
	mutex_lock(&lport->lp_mutex);
	fc_lport_enter_reset(lport);
...
/**
 * fc_lport_logo_resp2() - Handle response to LOGO request
 ...
 * and then unlock the lport.
 */
void fc_lport_logo_resp2(struct fc_seq *sp, struct fc_frame *fp,
			void *lp_arg)
{
	struct fc_lport *lport = lp_arg;
	u8 op;
...
EXPORT_SYMBOL(fc_lport_logo_resp2);
...
static void fc_lport_enter_logo(struct fc_lport *lport)
{
	...
	if (!lport->tt.elsct_send(lport, FC_FID_FLOGI, fp, ELS_LOGO,
				  fc_lport_logo_resp2, lport,
				  2 * lport->r_a_tov))
		fc_lport_error(lport, NULL);
}

/**
 * fc_lport_flogi_resp2() - Handle response to FLOGI request
 ...
 * and then unlock the lport.
 */
void fc_lport_flogi_resp2(struct fc_seq *sp, struct fc_frame *fp,
			 void *lp_arg)
{
	struct fc_lport *lport = lp_arg;
	struct fc_els_flogi *flp;
...
EXPORT_SYMBOL(fc_lport_flogi_resp2);
...
/**
 * fc_lport_config2() - Configure a fc_lport
 * @lport: The local port to be configured
 */
int fc_lport_config2(struct fc_lport *lport)
{
...
EXPORT_SYMBOL(fc_lport_config2);

/**
 * fc_lport_init2() - Initialize the lport layer for a local port
 * @lport: The local port to initialize the exchange layer for
 */
int fc_lport_init2(struct fc_lport *lport)
{
	if (!lport->tt.lport_recv)
		lport->tt.lport_recv = fc_lport_recv_req;

	if (!lport->tt.lport_reset)
		lport->tt.lport_reset = fc_lport_reset2;
...
/**
 * fc_lport_bsg_request2() - The common entry point for sending
 *			    FC Passthrough requests
 * @job: The BSG passthrough job
 */
int fc_lport_bsg_request2(struct fc_bsg_job *job)
{
...
EXPORT_SYMBOL(fc_lport_bsg_request2);

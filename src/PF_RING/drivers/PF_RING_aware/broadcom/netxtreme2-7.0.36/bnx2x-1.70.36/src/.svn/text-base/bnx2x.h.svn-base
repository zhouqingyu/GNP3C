...
#define DRV_MODULE_VERSION      "1.70.36" BNX2X_MOD_VER
#define DRV_MODULE_RELDATE      "$DateTime: 2011/12/01 05:35:22 $"
...
#define NON_ETH_CONTEXT_USE	(FCOE_PRESENT + OOO_CONTEXT_USE)
#endif
 ...
 */
#endif
#ifdef BCM_OOO /* non BNX2X_UPSTREAM */
...
#ifdef BCM_VF /* ! BNX2X_UPSTREAM */
#define IS_VF(bp)			(CHIP_NUM(bp) == CHIP_NUM_57712_VF || \
					 CHIP_NUM(bp) == CHIP_NUM_57800_VF || \
					 CHIP_NUM(bp) == CHIP_NUM_57810_VF || \
					 CHIP_NUM(bp) == CHIP_NUM_57840_VF)
...
#ifndef BNX2X_UPSTREAM /* ! BNX2X_UPSTREAM */
	/* niv ramrod can not be a part of func_rdata union because these
	 *  events might arrive in parallel to other events from func_rdata.
	 *  Therefore, if they would have been defined in the same union,
	 *  data can get coruupted.
	 */
	union {
		struct function_niv_update_data func_niv_update_rdata;
		struct niv_vif_list_ramrod_data func_niv_viflist_rdata;
	} func_niv_rdata;
#endif
...
#ifndef BNX2X_UPSTREAM /* ! BNX2X_UPSTREAM */
#define IS_MF_NIV(bp)		(bp->mf_mode == MULTI_FUNCTION_NIV)
#endif
...
#define BNX2X_MAX_TX_COS		2
	...
	int			disable_tpa;
#ifdef BNX2X_SAFC /* ! BNX2X_UPSTREAM */
#define BNX2X_COS_QUEUES(qs_per_cos, cos)	(((qs_per_cos) & (0xff << (cos)*8)) >> (cos)*8)
...
#endif
#ifndef BNX2X_UPSTREAM /* ! BNX2X_UPSTREAM */
	/* vntag: store default vlan used */
	int					vntag_def_vlan_tag;
#endif
	u32					pending_max;
...
#ifdef BCM_OOO
#define BNX2X_NUM_RX_QUEUES(bp)	(BNX2X_NUM_QUEUES(bp) - OOO_TX_CONTEXT_USE)
#else /* BNX2X_UPSTREAM */
...
#define BNX2X_MAX_HW_QUEUES(bp)  (bp->igu_sb_cnt - CNIC_PRESENT)
...
#else
/* skip rx queue if iscsi-ooo support is disabled and this is the rx ooo queue
 * OR if FCOE l2 support is diabled and this is the fcoe L2 queue
 */
#define skip_rx_queue(bp, idx)	((NO_ISCSI_OOO(bp) && IS_OOO_IDX(idx)) || \
				 (NO_FCOE(bp) &&  IS_FCOE_IDX(idx)) || \
				 IS_FWD_IDX(idx))

/* skip tx queue if iscsi-ooo support is disabled and this is the tx fwd queue
 * OR if FCOE l2 support is diabled and this is the fcoe L2 queue
 */
...
#ifdef BNX2X_IOV /* ! BNX2X_UPSTREAM */

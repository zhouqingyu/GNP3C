...
int bnx2x_dcb_get_lldp_params_ioctl(struct bnx2x *bp, void *uaddr)
{
	...
	if (!bp->port.pmf) {
		DP(NETIF_MSG_LINK, "Query is supported on pmf only\n");
		return -EINVAL;
	}

	if (bp->dcbx_enabled != BNX2X_DCBX_ENABLED_ON_NEG_ON) {
		DP(NETIF_MSG_LINK, "DCBX is not enabled\n");
		return -EINVAL;
	}
...
int bnx2x_dcb_set_dcbx_params_ioctl(struct bnx2x *bp, void *uaddr)
{
	...
	bp->dcb_state = dcbx_params.dcb_state;
	bp->dcbx_enabled = dcbx_params.dcbx_enabled;

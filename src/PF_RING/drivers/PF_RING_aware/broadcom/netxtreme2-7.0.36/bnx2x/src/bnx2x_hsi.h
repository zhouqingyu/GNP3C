...
struct shmem2_region {
	...
	#define DRV_FLAGS_DCB_CONFIGURATION_ABORTED     0x2
	#define DRV_FLAGS_DCB_MFW_CONFIGURED            0x4
...
struct fcoe_capabilities {
	u32 size; /* size of this structure (i.e. sizeof(oem_fcoe_features)) */
	u32 fcoe_settings;  /* FCoE Feature Settings */
	/* 0     FCoE Support 0b = Stateless Offload 1b = Stateful Offload */
	/* 7..1   Reserved */
	u32 max_ios;        /* Maximum number of I/Os per connection */
	u32 max_logins;     /* Maximum number of Logins per port  */
	u32 max_exchanges;  /* Maximum number of exchanges      */
	u32 max_npiv_wwn;   /* Maximum NPIV WWN per port         */
	u32 max_targets;    /* Maximum number of targets supported */
	u32 max_outstanding_cmds;   /* Maximum number of outstanding
...
/*  Per PCI Function Ethernet Statistics  */
struct ether_stat_info {
	u32 size; /* size of this structure (i.e. sizeof(ether_stat_info)) */
	u32 enabled;        /* 0 =Disabled, 1= Enabled */
	u8 version[12];     /* Function's Driver Version. padded to 12 */
	u8 mac_built_in[8]; /* Built-in MAC addr.   Big Endian EIU48:
			Leading bytes set to 0. Actual size is 6 bytes */
	u8 mac_local[8];    /* Locally Admin Addr.   BigEndian EIU48
...
#define FEATURE_ETHER_CHKSUM_OFFLOAD_MASK      0x01
#define FEATURE_ETHER_LSO_MASK                 0x02
#define FEATURE_ETHER_BOOTMODE_MASK             0x1C
#define FEATURE_ETHER_BOOTMODE_SHIFT        2
#define FEATURE_ETHER_BOOTMODE_NONE        (0x0 << 2)
#define FEATURE_ETHER_BOOTMODE_PXE         (0x1 << 2)
#define FEATURE_ETHER_BOOTMODE_ISCSI       (0x2 << 2)
#define FEATURE_ETHER_BOOTMODE_FCOE        (0x3 << 2)
#define FEATURE_ETHER_TOE_MASK                  0x20
	u32 lso_max_size;   /* LSO MaxOffloadSize. */
	u32 lso_min_seg_cnt;    /* LSO MinSegmentCount. */
	u32 ipv4_ofld_cnt;  /* Num Offloaded Connections TCP_IPv4. */
	u32 ipv6_ofld_cnt;  /* Num Offloaded Connections TCP_IPv6. */
	u32 promiscuous_mode;   /* Promiscuous Mode. non-zero true */
	u32 txq_size;       /* TX Descriptors Queue Size. Max Size (Items) */
	u32 rxq_size;       /* RX Descriptors Queue Size. Max Size (items) */
	u32 txq_avg_depth;  /* TX Descriptor Queue Avg Depth.
				   Percent % Avg Queue Depth since last poll */
	u32 rxq_avg_depth;  /* RX Descriptors Queue Avg Depth.
				   Percent % Avg Queue Depth since last poll */
	u32 rx_frames_lo; /* RX Frames received.  */
	u32 rx_frames_hi; /* RX Frames received.  */
	u32 rx_bytes_lo;  /* RX Bytes received.    */
	...
	u32 tx_bytes_lo;  /* TX Bytes sent.        */
	u32 tx_bytes_hi;  /* TX Bytes sent.        */
	u32 iov_offload;  /* IOV_Offload. 0=none; 1=MultiQueue, 2=VEB 3= VEPA*/
	u32 netq_cnt;     /* Number of NetQueue/VMQ Config'd. */
	u32 min_bw;     /* Function Min B/W Allocation Setting. */
	u32 max_bw;     /* Function Max B/W Rate Setting. Configured per CLP */
	u32 outer_vlan;     /* FlexNIC Outer VLAN ID (S-VID). VC Systems only */
	u32 vf_cnt;     /* Num VF assigned to this PF. */
	u32 perrcap;   /* PCI Errors-CAP ID*/
	u32 perrunce;  /* PCI Errors-Uncorrectable Error Status */
	u32 pcorumak;  /* PCI Errors-Uncorrectable Error Mask*/
	u32 perrcerr;  /* PCI Errors-Correctable Error Status*/
	u32 pcormask;  /* PCI Errors-Correctable Error Mask */
	u32 perrarcr;  /* PCI Errors-Advanced Error Capabilities and Ctrl Reg */
};

/*  Per PCI Function FCOE Statistics    */
struct fcoe_stat_info {
	u32 size; /* size of this structure (i.e. sizeof(fcoe_stat_info)) */
	u32 enabled;        /* Function Enabled / Type. 0 = Disabled
				   1 = NIC  2 = iSCSI 3=FCoE... */
	u8 version[12];     /* Function's Driver Version. */
	u8 mac_built_in[8]; /* Built-in MAC addr. */
	u8 mac_local[8];    /* Locally Admin Addr.   */
	u8 mac_add1[8];     /* Additional Programmed MAC Addr 1. */
	u8 mac_add2[8];     /* Additional Programmed MAC Addr 2. */
	u32 mtu_size;       /* MTU Size. Negotiated MTU            */
	u8 wwn_name[8];     /* World wide node name. Z terminated */
	u8 wwp_name[8];     /* World wide port name. Z terminated */
	u8 port_alias[8];   /* Port Alias. Z terminated         */
	u32 port_state;     /* Port State (Online or Loop Down).    1: Up */
	u32 txq_size;       /* FCoE TX Descriptors Queue Size. */
	u32 rxq_size;       /* FCoE RX Descriptors Queue Size. */
	u32 txq_avg_depth;  /* FCoE TX Descriptor Queue Avg Depth.
				   Avg Queue Depth since last poll */
	u32 rxq_avg_depth;  /* FCoE RX Descriptors Queue Avg Depth.
				   Avg Queue Depth since last poll */
	...
	u32 fcs_rx_errors;          /* receive packets with FCS Errors */
	u32 fc_crc_error_cnt;       /* FC frames with CRC errors. */
	u32 fip_login_failure_cnt;  /* FCoE/FIP Login failures. */
	u32 perrcap;   /* PCI Errors-CAP ID*/
	u32 perrunce;  /* PCI Errors-Uncorrectable Error Status */
	u32 pcorumak;  /* PCI Errors-Uncorrectable Error Mask*/
	u32 perrcerr;  /* PCI Errors-Correctable Error Status*/
...
/* Per PCI  Function iSCSI Statistics */
struct iscsi_stat_info {
	u32 size; /* size of this structure (i.e. sizeof(iscsi_stat_info)) */
	u32 enabled;        /* Function Enabled / Type. 0 = Disabled
				   1 = NIC     2 = iSCSI   3=FCoE... */
	u8 version[12];     /* Function's Driver Version. */
	u8 mac_built_in[8]; /* Built-in MAC addr.  */
	...
	u8 mac_add1[8];     /* Additional Programmed MAC Addr 1. */
	u32 qos_priority;     /* QoS Priority (per 802.1p).   0-7255 */
	u8 iscsi_boot_init_name[8]; /* iSCSI Boot Initiator Node name. */
	u8 wwp_name[8];     /* World wide port name. Z terminated */
	u8 iscsi_boot_target_name[8];   /* iSCSI Boot Target Name. */
	u32 iscsi_boot_target_ip;   /* iSCSI Boot Target IP. */
	u32 iscsi_boot_target_portal;   /* iSCSI Boot Target Portal. */
	u32 iscsi_boot_init_ip; /*iSCSI Boot Initiator IP Address. */
	...
	u32 rx_frames_lo;   /* iSCSI PDU RX Frames received.     */
	u32 rx_frames_hi;   /* iSCSI PDU RX Frames received.     */
	u32 rx_bytes_lo;    /* iSCSI PDU RX Bytes received.      */
	u32 rx_bytes_hi;    /* iSCSI PDU RX Bytes received.      */
	u32 tx_frames_lo;   /* iSCSI PDU TX Frames sent.         */
	u32 tx_frames_hi;   /* iSCSI PDU TX Frames sent.         */
...
enum common_spqe_cmd_id {
	...
	RAMROD_CMD_ID_COMMON_NIV_FUNCTION_UPDATE,
...
enum event_ring_opcode {
	...
	EVENT_RING_OPCODE_NIV_FUNCTION_UPDATE,

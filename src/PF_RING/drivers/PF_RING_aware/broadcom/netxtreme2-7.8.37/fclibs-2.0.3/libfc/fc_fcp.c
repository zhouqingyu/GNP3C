...
static struct kmem_cache *scsi_pkt_cachep2;
 ...
 */
int fc_queuecommand2(struct Scsi_Host *shost, struct scsi_cmnd *sc_cmd)
{
...
EXPORT_SYMBOL(fc_queuecommand2);
...
EXPORT_SYMBOL(fc_eh_abort2);

/**
 * fc_eh_device_reset2() - Reset a single LUN
 * @sc_cmd: The SCSI command which identifies the device whose
 *	    LUN is to be reset
 *
 * Set from SCSI host template.
 */
int fc_eh_device_reset2(struct scsi_cmnd *sc_cmd)
{
	struct fc_lport *lport;
	struct fc_fcp_pkt *fsp;
	struct fc_rport *rport = starget_to_rport(scsi_target(sc_cmd->device));
...
EXPORT_SYMBOL(fc_eh_device_reset2);

/**
 * fc_eh_host_reset2() - Reset a Scsi_Host.
 * @sc_cmd: The SCSI command that identifies the SCSI host to be reset
 */
int fc_eh_host_reset2(struct scsi_cmnd *sc_cmd)
{
...
EXPORT_SYMBOL(fc_eh_host_reset2);

/**
 * fc_slave_alloc2() - Configure the queue depth of a Scsi_Host
 * @sdev: The SCSI device that identifies the SCSI host
 *
 * Configures queue depth based on host's cmd_per_len. If not set
 * then we use the libfc default.
 */
int fc_slave_alloc2(struct scsi_device *sdev)
{
...
EXPORT_SYMBOL(fc_slave_alloc2);

/**
 * fc_change_queue_depth2() - Change a device's queue depth
 * @sdev:   The SCSI device whose queue depth is to change
 * @qdepth: The new queue depth
 * @reason: The resason for the change
 */
int fc_change_queue_depth2(struct scsi_device *sdev, int qdepth, int reason)
{
	switch (reason) {
...
EXPORT_SYMBOL(fc_change_queue_depth2);

/**
 * fc_change_queue_type2() - Change a device's queue type
 * @sdev:     The SCSI device whose queue depth is to change
 * @tag_type: Identifier for queue type
 */
int fc_change_queue_type2(struct scsi_device *sdev, int tag_type)
{
	if (sdev->tagged_supported) {
...
EXPORT_SYMBOL(fc_change_queue_type2);

/**
 * fc_fcp_destory2() - Tear down the FCP layer for a given local port
 * @lport: The local port that no longer needs the FCP layer
 */
void fc_fcp_destroy2(struct fc_lport *lport)
{
	struct fc_fcp_internal *si = fc_get_scsi_internal(lport);
...
EXPORT_SYMBOL(fc_fcp_destroy2);

int fc_setup_fcp(void)
{
	int rc = 0;

	scsi_pkt_cachep2 = kmem_cache_create("libfc_fcp_pkt2",
...
{
	if (scsi_pkt_cachep2)
		kmem_cache_destroy(scsi_pkt_cachep2);
}

/**
 * fc_fcp_init2() - Initialize the FCP layer for a local port
 * @lport: The local port to initialize the exchange layer for
 */
int fc_fcp_init2(struct fc_lport *lport)
{
	int rc;
	struct fc_fcp_internal *si;
	...
	si->scsi_pkt_pool = mempool_create_slab_pool(2, scsi_pkt_cachep2);
	if (!si->scsi_pkt_pool) {
		rc = -ENOMEM;
		goto free_internal;
	}

...
#define DRV_MODULE_RELDATE	"Apr 30, 2013"
		...
		struct packet_type *ptype, struct net_device *olddev)
{
	...
	fr = fcoe_dev_from_skb(skb);
	fr->fr_dev = lport;
#if !defined(__BNX2FC_SLES11SP2__) && !defined(__BNX2FC_RHEL62__)
	fr->ptype = ptype;
#endif
 ...
 * @vlan_id:	vlan id
 *
 * Handles NETDEV_UP, NETDEV_DOWN, NETDEV_GOING_DOWN,NETDEV_CHANGE
 * NETDEV_CHANGE_MTU and NETDEV_UNREGISTER events
 */
...
out:
#else //#ifdef __BNX2FC_SLES11SP3__
	ctlr = bnx2fc_to_ctlr(interface);
...
#ifdef __BNX2FC_SLES11SP3__
			memcpy(ctlr->ctl_src_addr, ha->addr, ETH_ALEN);
#else
...
static struct bnx2fc_hba *bnx2fc_hba_create(struct cnic_dev *cnic)
{
	...
	hba->max_outstanding_cmds = hba->elstm_xids = (hba->max_tasks / 2);

				   ...
				   union bnx2x_classification_ramrod_data *data)
{
	...
	DP(BNX2X_MSG_SP, "Checking VLAN_MAC ("BNX2X_MAC_FMT", %d) for ADD "
			 "command\n", BNX2X_MAC_PRN_LIST(data->vlan_mac.mac),
			 data->vlan_mac.vlan);
				 ...
				 union bnx2x_classification_ramrod_data *data)
{
	...
	DP(BNX2X_MSG_SP, "Checking VLAN_MAC ("BNX2X_MAC_FMT", %d) for DEL "
			 "command\n", BNX2X_MAC_PRN_LIST(data->vlan_mac.mac),
			 data->vlan_mac.vlan);
			       ...
			       enum bnx2x_func_cmd cmd)
{
	 ...
	 * legal completion.
	 */
	return bnx2x_func_state_change_comp(bp, o, cmd);
}
				     ...
				     struct bnx2x_func_state_params *params)
{
	...
	case BNX2X_F_STATE_STARTED:
		...
		/* NIV ramrods can be sent only in started mode, and only
		 * if not pending for function_stop ramrod completion
		 * for these events - next state remained STARTED.
		 */
		else if ((cmd == BNX2X_F_CMD_NIV_UPDATE) &&
			 (!test_bit(BNX2X_F_CMD_STOP, &o->pending)))
			next_state = BNX2X_F_STATE_STARTED;

		else if ((cmd == BNX2X_F_CMD_NIV_VIFLISTS) &&
			 (!test_bit(BNX2X_F_CMD_STOP, &o->pending)))
			next_state = BNX2X_F_STATE_STARTED;
#endif
...
static inline int bnx2x_func_send_niv_update(struct bnx2x *bp,
					 struct bnx2x_func_state_params *params)
{
	struct bnx2x_func_sp_obj *o = params->f_obj;
	struct function_niv_update_data *rdata =
		(struct function_niv_update_data *)o->niv_rdata;
	dma_addr_t data_mapping = o->niv_rdata_mapping;
	struct bnx2x_func_niv_update_params *niv_update_params =
		&params->params.niv_update;

	memset(rdata, 0, sizeof(*rdata));

	/* Fill the ramrod data with provided parameters */
	rdata->vif_id = cpu_to_le16(niv_update_params->vif_id);
	rdata->niv_default_vlan =
		cpu_to_le32(niv_update_params->niv_default_vlan);
	rdata->allowed_priorities = niv_update_params->allowed_priorities;
	...
	DP(BNX2X_MSG_SP, "vntag: sending ramrod func_update, "
			 "vif_id 0x%x dvlan 0x%x prio 0x%x\n",
			 rdata->vif_id, rdata->niv_default_vlan,
			 rdata->allowed_priorities);

	return bnx2x_sp_post(bp, RAMROD_CMD_ID_COMMON_NIV_FUNCTION_UPDATE, 0,
			     U64_HI(data_mapping),
			     U64_LO(data_mapping), NONE_CONNECTION_TYPE);
}

static inline int bnx2x_func_send_niv_viflists(struct bnx2x *bp,
					 struct bnx2x_func_state_params *params)
{
	struct bnx2x_func_sp_obj *o = params->f_obj;
	struct niv_vif_list_ramrod_data *rdata =
		(struct niv_vif_list_ramrod_data *)o->niv_rdata;
	struct bnx2x_func_niv_viflists_params *niv_viflist_params =
		&params->params.niv_viflists;
	u64 *p_rdata = (u64 *)rdata;

	memset(rdata, 0, sizeof(*rdata));

	/* Fill the ramrod data with provided parameters */
	rdata->vif_list_index       = niv_viflist_params->vif_list_index;
	rdata->func_bit_map         = niv_viflist_params->func_bit_map;
	rdata->niv_vif_list_command = niv_viflist_params->niv_vif_list_command;
	rdata->func_to_clear        = niv_viflist_params->func_to_clear;

	/* send in echo type of sub command */
	rdata->echo = niv_viflist_params->niv_vif_list_command;
	...
	DP(BNX2X_MSG_SP, "vntag: sending ramrod lists, cmd 0x%x index 0x%x "
			 "func_bit_map 0x%x func_to_clr 0x%x\n",
			 rdata->niv_vif_list_command, rdata->vif_list_index,
			 rdata->func_bit_map, rdata->func_to_clear);

	/* this ramrod sends data directly and not through DMA mapping */
	return bnx2x_sp_post(bp, RAMROD_CMD_ID_COMMON_NIV_VIF_LISTS, 0,
...
#ifndef BNX2X_UPSTREAM /* ! BNX2X_UPSTREAM */
	case BNX2X_F_CMD_NIV_UPDATE:
		return bnx2x_func_send_niv_update(bp, params);
	case BNX2X_F_CMD_NIV_VIFLISTS:
		return bnx2x_func_send_niv_viflists(bp, params);
#endif
...
void bnx2x_init_func_obj(struct bnx2x *bp,
			 ...
			 void *rdata, dma_addr_t rdata_mapping,
#ifndef BNX2X_UPSTREAM /* ! BNX2X_UPSTREAM */
			 void *niv_rdata, dma_addr_t niv_rdata_mapping,
#endif
...
#ifndef BNX2X_UPSTREAM /* ! BNX2X_UPSTREAM */
	obj->niv_rdata = niv_rdata;
	obj->niv_rdata_mapping = niv_rdata_mapping;
#endif

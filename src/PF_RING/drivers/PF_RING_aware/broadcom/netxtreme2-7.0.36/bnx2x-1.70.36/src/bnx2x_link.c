			 ...
			 const struct bnx2x_ets_params *ets_params)
{
	...
	if ((ets_params->num_of_cos > max_num_of_cos)) {
		DP(NETIF_MSG_LINK,
		   "bnx2x_ets_E3B0_config the number of COS isn't supported\n");
		return -EINVAL;
	}
...
int bnx2x_link_update(struct link_params *params, struct link_vars *vars)
{
	...
	if (params->feature_config_flags &
	    FEATURE_CONFIG_BC_SUPPORTS_VNTAG)
		bnx2x_fw_command(bp,
...
int bnx2x_init_emul(struct link_params *params,
			       struct link_vars *vars)
{
	struct bnx2x *bp = params->bp;
	if (CHIP_IS_E3(bp)) {
...
int bnx2x_init_fgpa(struct link_params *params,
			       struct link_vars *vars)
{
	/* enable on E1.5 FPGA */
	struct bnx2x *bp = params->bp;
...
int bnx2x_phy_init(struct link_params *params, struct link_vars *vars)
{
	...
	if (CHIP_REV_IS_FPGA(bp)) {
		return bnx2x_init_fgpa(params, vars);
	} else
	if (CHIP_REV_IS_EMUL(bp)) {

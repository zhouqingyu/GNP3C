...
struct fc_frame *fc_frame_alloc_fill2(struct fc_lport *, size_t payload_len);
struct fc_frame *_fc_frame_alloc2(size_t payload_len);
...
static inline struct fc_frame *fc_frame_alloc(struct fc_lport *dev, size_t len)
{
	 ...
	 */
	if (len && len % 4)
		fp = fc_frame_alloc_fill2(dev, len);
	else
		fp = _fc_frame_alloc2(len);
	return fp;
}
 ...
 */
u32 fc_frame_crc_check2(struct fc_frame *);

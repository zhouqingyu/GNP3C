...
static inline void bnx2x_move_fp(struct bnx2x *bp, int from, int to)
{
	...
	/* move tpa_info contents; As each fp's tpa_info is allocated
	 * independently, we also need to free the current resident,
	 * as well as fix the counter (used only for freeing the tpa_infos)
	 */
	kfree(from_fp->tpa_info);
...
#elif (VMWARE_ESX_DDK_VERSION >= 50000) && (!DYNAMIC_NETQ_ALLOC)
			      ...
			      u32 xmit_type)
{
	...
	if (xmit_type & XMIT_GSO_V4) {
		...
		/* GSO on 57710/57711 need FW to calculate IP checksum */
...
#if defined(__VMKLNX__) && defined(VMWARE_ESX_DDK_VERSION) && (VMWARE_ESX_DDK_VERSION < 50000) /* ! BNX2X_UPSTREAM */
...
#if defined(__VMKLNX__) && (DYNAMIC_NETQ_ALLOC) /* ! BNX2X_UPSTREAM */
int esx_alloc_fp_mem_at(struct bnx2x *bp, int index, int alloc_skb)
#else /* BNX2X_UPSTREAM */
